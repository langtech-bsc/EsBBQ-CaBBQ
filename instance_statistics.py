# Run this code from the main bbq directory!
...
for file in sorted(os.listdir("data_es/stats")):
    file_path = f"data_es/stats/{file}"
...
# Add labels to points with instances > 1000
for i, row in all_data.iterrows():
    if row['instances'] > 1000:
...
# Save plot
plt.savefig(f"instance_statistics/{current_date}_template-fertility.png", format='png', bbox_inches='tight', dpi=300)
...
for file in sorted(os.listdir("data_es")):
    if file.endswith(".full.csv"):
        ...
        gender_category_dict[category] = {
                                            'm': (df['gender_category'] == 'm').sum(),
                                            'f': (df['gender_category'] == 'f').sum(),
                                            'n': (df['gender_category'] == 'n').sum(),
                                            'm vs. f': (df['gender_category'] == 'm vs. f').sum(),
                                            'm_and_f': (df['gender_category'] == 'm_and_f').sum(),
                                            'm_and_f_and_n': (df['gender_category'] == 'm_and_f_and_n').sum(),
                                            'total': len(df)
                                        }
        
        if gender_category_dict[category]['total'] != sum(gender_category_dict.values())-gender_category_dict['total']:
        ...
        with open(f"instance_statistics/{current_date}_social-groups_{category}.csv",mode="w",newline='') as groups_file:
...
with open(f"instance_statistics/{current_date}_len-info.csv", mode='w', newline='') as len_file:
...
with open(f"instance_statistics/{current_date}_gender-info.csv", mode='w', newline='') as gender_file:
...
# Save plot
plt.savefig(f"instance_statistics/{current_date}_gender-info.png", format='png', bbox_inches='tight', dpi=300)
...
# Save plot
plt.savefig(f"instance_statistics/{current_date}_social-groups.png", format='png', bbox_inches='tight', dpi=300)